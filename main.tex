...
\begin{document}
    ...
    \subsubsection{Isomorphism with Euclidean Spaces}
    ...
    \begin{definition}[Alternative Definition for Addition]\label{def:curve-add-alt}
        ...
        \]
    \end{definition}
    \begin{definition}[Alternative Definition of Basis Tangent Vectors]\label{def:unit-tangent-alt}
        \[
    ...
    \begin{definition}[Basis Tangent Vectors]\label{def:basis-derivations}
        ...
        C^\infty\). For the use of \(u^\mu\), see \cref{def:coordinate-functions}.
    \end{definition}
    \begin{theorem}
        \[
        \tvec{\mu}{p}x^\nu = \delta\indices{^\nu_\mu}.
    ...
    \begin{theorem}[Isomorphism of Curves and Derivations]\label{thm:curve-derivation-iso}
        Similar to \cref{def:directional-derivative-curve}, we define the linear 
    ...
    \begin{proof}
        ...
        \end{aligned}
        \]
        (surjectivity) To show surjectivity, we need 
    ...
    \end{proof}
    \begin{remark}
        It's tempting to use the powerful theorem that surjectivity of linear 
        transformations is equivalent to injectivity. However, the requirement 
        of that theorem is that \textbf{the dimensions of the two spaces are 
        equal}. At this point, we don't know the dimension of \(T_p\manifold[M]\), 
    ...
    \begin{proof}
        ...
        \end{center}
        Since \(\ell_p^\phi\) and \(\ell_{h(p)}^\psi\) are linear, to prove 
        \(h_*\) is linear, we need only to show \(\ell_{h(p)}^\psi \circ 
        h_* \circ \ell_p^\phi\) is linear. \par
        \[
        \begin{aligned}
            &\ell_{h(p)}^\psi \circ h_* \circ \ell_p^\phi(v) \\
            =&\ell_{h(p)}^\psi \circ h_*([\phi ^{-1}(\phi(p)+tv)]) \\
            =&\ell_{h(p)}^\psi([h\circ\phi ^{-1}(\phi(p)+tv)]) \\
