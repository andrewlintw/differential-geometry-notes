...
\begin{document}
    ...
    \begin{definition}[Pseudoforms to Forms]\label{def:pseudo-to-form}
        For a pseudoform \(\pseudoform{\omega} \in \tilde{\Lambda}^k(\manifold[M])\) 
        on an orientable manifold \(\manifold[M]\) of degree \(m\) positively oriented 
        by the atlas \(\Phi_+\), we define its underlying real form by letting them 
        agree on the positive charts, that is,
        \[
        \omega_\phi := o(\phi)\pseudoform{\omega}_\phi,\; \forall \phi \in \Phi, 
        \]
        where 
        \[
        o(\phi) := \begin{cases}
        1 & \forall \phi \in \Phi_+ \\
        -1 & \forall \phi \in \Phi_-
        \end{cases}
        \]
    \end{definition}
    \begin{definition}[Forms to Pseudoforms]\label{def:form-to-pseudo}
        For a form \(\omega \in \form{k}{M}\) on an orientable manifold 
        \(\manifold[M]\) of degree \(m\), we define its corresponding pseudoform by 
        taking its positive value. That is, 
        \[
        \pseudoform{\omega}_\phi := o(\phi)\omega_\phi,\; \forall \phi \in \Phi, 
        \]
    \end{definition}
    ...
    \begin{remark}
        ...
        \begin{aligned}
            \eta &:= f^*\omega = dx^2 \\
            \pseudoform{\eta} &= dx^2 \\
            \int_{\manifold[Z]} \pseudoform{\eta} &= 1.
        \end{aligned}
        \]
