...
\label{def:The-Exponential-Map}
    ...
    G\) for \(X \in \lalg{g}\). Then we define the exponential map \(\exp
    : \lalg{g} \to G\) by 
    \[
    \exp(X) = \phi_X(1).
    \]
...
\begin{proof}
    ...
    \[
        \begin{aligned}
            [\det(I+tX)]_{t=0} &= \eval*{\frac{d}{dt}\det(I+tX)}_{t=0} \\
                              &=\eval*{\frac{d}{dt}\left( 1+\tr(X)t + O(t^2) \right) }_{t=0}\\
                              &= \tr(X).
        \end{aligned}
    \]
    ...
    \]
\end{proof}
\begin{theorem}[Lie Algebras of Classical Groups]
\label{thm:Lie-Algebras-of-Classical-Groups}
    The Lie algebras of the special linear groups are
    \[
        \begin{aligned}
            \spli{n}{\reals} &= \Set*{M \in M_{n\times
            n}(\reals)}{\tr(M)=0} \\
            \spli{n}{\complex} &= \Set*{M \in M_{n\times
        n}(\complex)}{\tr(M)=0}.
        \end{aligned}
    \]
    The Lie algebras of the orthogonal and unitary groups are
    \[
        \begin{aligned}
        \orth{n}{\reals} &= \Set*{M \in M_{n \times
        n}(\reals)}{M+\transpose{M} = 0} \\
        \lalg{u}(n) &= \Set*{M \in M_{n \times n}(\complex)}{M +
        \hermitian{M} = 0}.
        \end{aligned}
    \]
    The Lie algebras of the special orthogonal and special unitary groups
    are
    \[
    \begin{aligned}
        \lalg{so}(n) &= \lalg{o}(n) \\
    \lalg{su}(n) &= \Set*{M \in M_{n \times n}(\complex)}{M +
    \hermitian{M} = 0, \tr M = 0}.
    \end{aligned}
    \]
\end{theorem}
