...
\begin{document}
    ...
    \begin{theorem}[Evaluate k-forms with Vector Fields]\label{thm:evaluate-forms}
        On a smooth manifold \(\manifold[M]\), under the chart \(\phi = (x^1, 
        \dots, x^m)\), let \(k\) vector fields have local representation 
        \(X_b = \xi\indices{^a_b}\tfld{a}\). Then 
        \[
        (dx^1\wedge\dots\wedge dx^k)(X_1, \dots, X_k) = \det \xi\indices{^a_b}.
        \]
    \end{theorem}
    \begin{proof}
        \[
        \begin{aligned}
            &(dx^1\wedge\dots\wedge dx^k)(X_1, \dots, X_k)\\
            =&(dx^1\wedge\dots\wedge dx^k)(\xi\indices{^{i_1}_1}\tfld{i_1}, \dots, 
            \xi\indices{^{i_k}_k}\tfld{i_k})\\
            =&\xi\indices{^{i_1}_1}\xi\indices{^{i_k}_k}
            (dx^1\wedge\dots\wedge dx^k)(\tfld{i_1}, \dots, \tfld{i_k}).
        \end{aligned}
        \]
        ...
        \begin{aligned}
            &\xi\indices{^{i_1}_1}\xi\indices{^{i_k}_k}
            (dx^1\wedge\dots\wedge dx^k)(\tfld{i_1}, \dots, \tfld{i_k})\\
            =&\sum_{\sigma \in S_k} \xi\indices{^{\sigma(1)}_1}
            \xi\indices{^{\sigma(k)}_k}(dx^1\wedge\dots\wedge dx^k)
            (\tfld{\sigma(1)}, \dots, \tfld{\sigma(k)}).
        \end{aligned}
        \]
        ...
        \begin{aligned}
            ...
            \xi\indices{^{\sigma(k)}_k}(dx^1\wedge\dots\wedge dx^k)
            (\tfld{\sigma(1)}, \dots, \tfld{\sigma(k)})\\
            =&\sum_{\sigma \in S_k} (\sgn \sigma)\xi\indices{^{\sigma(1)}_1}\dots
            \xi\indices{^{\sigma(k)}_k}\\
            =&\det \xi\indices{^a_b}.
        \end{aligned}
        \]
    \end{proof}
    \begin{corollary}[Evaluate k-forms with Vector Fields]\label{cor:evaluate-forms}
        On a smooth manifold \(\manifold[M]\), let \(k\) covectors \(\omega^i\) and 
        \(k\) vector fields \(X_i\), \(1 \leq i \leq k\), and let the matrix 
        \(\xi\indices{^a_b}=\omega^a(X_b)\). Then
        \[
        (\omega^1\wedge\dots\wedge \omega^k)(X_1, \dots, X_k) = \det \xi\indices{^a_b}.
        \]
    \end{corollary}
    \begin{proof}
        This is easily proved if we use row-linearity of determinants, for 
    ...
    \end{definition}
    \begin{theorem}
        A useful formula for the evaluation of the exterior derivative of 
        a 0-form \(f \in C^\infty(\manifold[M])\) is the following,
        \[
        df(X) := \lder{X}f = Xf.
        \]
    \end{theorem}
    \begin{proof}
        ...
        So
        \[
        df(X) = (\tfld{\mu}f)(dx^\mu)(X) = X^\mu(\tfld{\mu}f)
        =(X^\mu\tfld{\mu})f=Xf.
        \]
    \end{proof}
    \begin{theorem}[Invariant Formula of Exterior Derivative for 1-forms]\label{thm:exterior-derivative-one-form}
    ...
    \end{theorem}
    \begin{proof}
        By \cref{thm:exterior-derivative-one-form}, 
        \[
